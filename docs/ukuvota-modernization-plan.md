...
- **Anonymous/alias participation:** current proposal/vote actions require an account; target allows unlogged users to vote with name/alias, with optional account-backed identity.
...
### Phase 5: Proposal Collection And Templates
...
3. For anonymous users, derive a stable local voter key per process from local settings, while still asking for display name/persistent alias.
...
6. Update remote API contract if the server currently requires auth tokens for votes.
...
### Phase 7: Results And Legacy Exports
...
### Phase 9: Manual, Localization, And Copy
...
3. Smoke test these paths: empty dashboard, create wizard, proposal phase, template proposal, anonymous vote, logged-in vote, duplicate alias rejection, results, export, completed dashboard.
...
6. Uku/server: anonymous vote support.
7. Uku: voting UI and results/export parity.
8. Uku: dashboard polish, manual/localization, final verification.
